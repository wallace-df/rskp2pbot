...
export default {
  ...
  created() {
    this.$store.commit("setActiveSection", "AdminReleasePage");
    this.loadParams();
  },
  ...
  methods: {
    async loadParams() {
      ...
      try {
        ...
        this.amount = this.formatAmount(order.amount, this.getToken(order.tokenContractAddress));
    ...
    async release() {
      ...
      try {
        ...
        await walletInstance.contract.methods.adminReleaseToBuyer(this.orderId).send({from: walletInstance.walletAddress});
